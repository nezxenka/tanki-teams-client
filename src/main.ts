...
const UPDATE_CONFIG_TEMPLATE_URL =
    "https://tanks.aleskysha.ru/update-config-template.json";
const TANKI_ONLINE_URL = "https://tankionline.com/play/";

const EXTERNAL_BROWSER_URLS = [
    "https://tanks.aleskysha.ru",
    "https://github.com/nezxenka"
];
const TANKI_TEAMS_EXTENSION_KEY = "";
...
async function updateAndLoadExtensions(userDataPath: string) {
    ...
    // Only auto-load extension if key is configured
    if (TANKI_TEAMS_EXTENSION_KEY &&
        !extensions.some((extension) =>
            extension.manifest.key === TANKI_TEAMS_EXTENSION_KEY)) {
        const tweaksExtension = {
            path: Path.join(extensionsPath, "tanki-teams"),
            manifest: {
                key: TANKI_TEAMS_EXTENSION_KEY
            }
        };
